...
void mqtt_pktype_ctl(uint8_t packtype, uint8_t *buf, uint32_t buflen)
{
	...
	case PUBLISH:
		// 拆析PUBLISH消息
		if (MQTTDeserialize_publish(&msg.dup, &msg.qos, &msg.retained, &msg.id, &receivedTopic,
...
MQTT_START:
	...
	// 订阅消息
	if (MQTTSubscribe(MQTT_Socket, (char *)TOPIC_SUBSCRIBE, QOS1) < 0)
	{
		// 重连服务器
...
void mqtt_send_thread(void *pvParameters)
{
	...
	/* power 为字符串型属性；temp 为数值型，与物模型 JSON 一致 */
	char power_str[] = "789";
	double temp_val = 27.4;
MQTT_SEND_START:
	...
	while (1)
	{
		...
		if (xReturn == pdTRUE)
		{
			...
			//        printf("a = %f,b = %f\n",a,b);
			// 更新数据
			res = cJSON_Update(cJSON_Data, POWER_STM32, power_str);
			res = cJSON_Update(cJSON_Data, TEMP_STM32, &temp_val);
			...
			if (UPDATE_SUCCESS == res)
			{
				...
				// 发布消息
				ret = MQTTMsgPublish(MQTT_Socket, (char *)TOPIC_PUBLISH, QOS0, (uint8_t *)p);
				if (ret >= 0)
				{
					// 表明有数据交换
					no_mqtt_msg_exchange = 0;
